...
SCENARIO("Track is parallel to the axis OZ")
{
        ...
        "Let dx = index, dy = index / 2, dz = 0\n")
  {
    ...
    cudaMemcpy(
      dev_muon_hits, muon_hits_events.data(), muon_hits_events.size() * sizeof(Muon::HitsSoA), cudaMemcpyHostToDevice);
    ...
    WHEN("Track parallel to the axis OZ (x=9, y=9, z=0, dx=0, dy=0)")
    {
           ...
           "\t station 4 - 300 \n")
      {
        ...
        for (int i_station = 0; i_station < Muon::Constants::n_stations; i_station++) {
          ...
          CHECK_THAT(
            host_features[offset::RES_X + i_station],
            Catch::Matchers::WithinAbs(
              (extrapolation_x[i_station] - muon_hits_events[0].x[closest_idx]) /
                sqrt(
                  closest_idx * closest_idx * INVSQRT3 * INVSQRT3 +
                  multiple_scattering_error * multiple_scattering_error),
              eps));
          CHECK_THAT(
            host_features[offset::RES_Y + i_station],
            Catch::Matchers::WithinAbs(
              (extrapolation_y[i_station] - muon_hits_events[0].y[closest_idx]) /
                sqrt(
                  closest_idx * closest_idx * INVSQRT3 * INVSQRT3 / 4 +
                  multiple_scattering_error * multiple_scattering_error),
              eps));
        }
      }
    ...
    WHEN("Track parallel to the axis OZ (x=5, y=5, z=0, dx=0, dy=0)")
    {
           ...
           "\t station 4 - 300 \n")
      {
        ...
        for (int i_station = 0; i_station < Muon::Constants::n_stations; i_station++) {
          const float multiple_scattering_error = COMMON_FACTOR * trav_dist[i_station] * sqrt(trav_dist[i_station]);
          CHECK(
            any_of(closest_hits[i_station], host_features[offset::DTS + i_station], muon_hits_events[0].delta_time) ==
            true);
          CHECK(
            any_of(
              closest_hits[i_station], host_features[offset::TIMES + i_station], (int*) muon_hits_events[0].time) ==
            true);
          CHECK(
            any_of(
              closest_hits[i_station], 2 - host_features[offset::CROSS + i_station], muon_hits_events[0].uncrossed) ==
            true);
          ...
          CHECK_THAT(
            host_features[offset::RES_X + i_station],
            Catch::Matchers::WithinAbs(true_res_x[0], eps) || Catch::Matchers::WithinAbs(true_res_x[1], eps) ||
          ...
          CHECK_THAT(
            host_features[offset::RES_Y + i_station],
            Catch::Matchers::WithinAbs(true_res_y[0], eps) || Catch::Matchers::WithinAbs(true_res_y[1], eps) ||
