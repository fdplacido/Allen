...
SCENARIO("General case")
{
  ...
  const std::vector<size_t> MUON_COMMON_HITS_SIZES = {
    26320, 2320, 10096, 14560, 11344, 4096, 12544, 7360, 14368, 18160};
  ...
  for (size_t i = 0; i < DATA_FILES.size(); i++) {
    ...
    for (int it = 0; it < 100; it++) {
      muonRawToHitsDecode(
        muon_raw_raw_input.data(), muon_raw_offsets, MUON_RAW_SIZES[i], 2, actual_vector, &muonRawToHits);
    }
    ...
    for (int j = 0; j < Muon::Constants::n_stations; j++) {
      ...
      for (int k = expected.station_offsets[j];
           k < expected.station_offsets[j] + expected.number_of_hits_per_station[j];
           k++) {
        expected_tiles.push_back(expected.tile[k]);
      }
      std::vector<int> actual_tiles;
      for (int k = actual.station_offsets[j]; k < actual.station_offsets[j] + actual.number_of_hits_per_station[j];
           k++) {
