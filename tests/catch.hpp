...
namespace Catch {
  ...
  struct IExceptionTranslator {
    ...
    virtual std::string translate(ExceptionTranslators::const_iterator it, ExceptionTranslators::const_iterator itEnd)
      const = 0;
  };
  ...
  class ExceptionTranslatorRegistrar {
    ...
    public:
      ...
      std::string translate(ExceptionTranslators::const_iterator it, ExceptionTranslators::const_iterator itEnd)
        const override
      {
        try {
          if (it == itEnd)
...
namespace Catch {
  namespace Generators {
    ...
    public:
      RangeGenerator(T const& first, T const& last) : m_first(first), m_last(last) { assert(m_last > m_first); }
    ...
    public:
      Generator(size_t size, std::unique_ptr<IGenerator<T>> generator) : m_generator(std::move(generator)), m_size(size)
      {}
    ...
    public:
      GeneratorRandomiser(Generator<T>&& baseGenerator, size_t numberOfItems) :
        m_baseGenerator(std::move(baseGenerator)), m_indices(randomiseIndices(numberOfItems, m_baseGenerator.size()))
      {}
...
namespace Catch {
  namespace Detail {
    ...
    {
      CATCH_ENFORCE(
        margin >= 0, "Invalid Approx::margin: " << margin << '.' << " Approx::Margin has to be non-negative.");
      m_margin = margin;
    }
...
namespace Catch {
  ...
  namespace Generators {
    struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
      ...
      {
        TrackerBase::close();
        if (m_runState == CompletedSuccessfully && m_index < m_generator->size() - 1) m_runState = Executing;
      }
...
namespace Catch {
  ...
  ConsoleReporter::ConsoleReporter(ReporterConfig const& config) :
    StreamingReporterBase(config), m_tablePrinter(new TablePrinter(
                                     config.stream(),
