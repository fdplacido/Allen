...
{
  return ((p & VeloClustering::mask_top) << 1) | ((p & VeloClustering::mask_bottom) >> 1) |
         ((p & VeloClustering::mask_bottom_right) << 15) | ((p & VeloClustering::mask_top_left) >> 15) | (p >> 16) |
         (p >> 17) | (p << 16) | (p << 17);
}
...
{
  return ((p & VeloClustering::mask_ltr_top_right) >> 15) | (p >> 16) | (p >> 17);
}
...
{
  return ((p & VeloClustering::mask_rtl_bottom_left) << 15) | (p << 16) | (p << 17);
}
...
{
  return ((p & VeloClustering::mask_top) << 1) | ((p & VeloClustering::mask_bottom_right) << 15) | (p << 16) |
         (p << 17);
}
...
{
  return ((p & VeloClustering::mask_bottom) >> 1) & ((p & VeloClustering::mask_top_left) >> 15) & (p >> 16) & (p >> 17);
}

__device__ uint32_t candidates_current_mask_with_right_clusters(uint32_t p, uint32_t rp)
{
  return ((p & VeloClustering::mask_bottom) >> 1) & (((p & VeloClustering::mask_top_left) >> 15) | (rp << 17)) &
         ((p >> 16) | (rp << 16)) & ((p >> 17) | ((rp & VeloClustering::mask_rtl_bottom_left) << 15));
}
  ...
  float* dev_velo_sp_fy)
{
  ...
  for (int i = 0; i < (number_of_candidates + blockDim.x - 1) / blockDim.x; ++i) {
    ...
    if (candidate_number < number_of_candidates) {
      ...
      for (int clustering_iterations = 0; clustering_iterations < VeloClustering::max_clustering_iterations;
      ...
      const int n = __popc(cluster[0]) + __popc(cluster[1]);
      ...
      if (do_store) {
        ...
        const uint32_t transposed_clusters[4] = {(cluster[0] & 0x000F000F) | ((cluster[1] & 0x000F000F) << 4),
