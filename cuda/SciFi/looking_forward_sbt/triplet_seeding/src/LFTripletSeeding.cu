  ...
  const uint8_t relative_middle_layer)
{
  ...
  for (int i = blockIdx.y; i < ut_event_number_of_tracks; i += gridDim.y) {
    ...
    __syncthreads();
    for (uint8_t j = threadIdx.x; j < 64; j += blockDim.x) {
      best_chi2[j] = max_chi2;
    ...
    {
      const auto candidate_h0_offset = dev_scifi_lf_number_of_candidates[current_track_index * LookingForward::number_of_x_layers + relative_middle_layer - 1];
      const auto candidate_h1_offset = dev_scifi_lf_number_of_candidates[current_track_index * LookingForward::number_of_x_layers + relative_middle_layer];
      const auto candidate_h2_offset = dev_scifi_lf_number_of_candidates[current_track_index * LookingForward::number_of_x_layers + relative_middle_layer + 1];
      ...
      const uint8_t candidate_h2_size = dev_scifi_lf_number_of_candidates[current_track_index * LookingForward::number_of_x_layers + relative_middle_layer + 2] - candidate_h2_offset;
    ...
    for (uint8_t j=threadIdx.x; j<64; j+=blockDim.x) {
      if (best_h0_h2[j] != -1) {
        ...
        scifi_tracks[current_insert_index] = SciFi::Track{
          dev_scifi_lf_candidates[h0_candidate_offset + best_h0_h2[j]],
          dev_scifi_lf_candidates[h1_candidate_offset + j],
          dev_scifi_lf_candidates[h2_candidate_offset + best_h0_h2[64 + j]],
          best_chi2[j]
        };
      }
    }
