  ...
  int* dev_atomics_scifi)
{
  ...
  __shared__ int8_t best_h0_h2[4 * 2 * LookingForward::maximum_number_of_candidates];
  ...
  for (uint16_t i = blockIdx.y; i < ut_event_number_of_tracks; i += gridDim.y) {
    ...
    for (uint16_t j = threadIdx.x; j < 4 * LookingForward::maximum_number_of_candidates; j += blockDim.x) {
      best_chi2[j] = LookingForward::chi2_mean_triplet_single + 2.5f * LookingForward::chi2_stddev_triplet_single;
    ...
    for (uint8_t j = threadIdx.x; j < LookingForward::maximum_number_of_candidates_per_ut_track; j += blockDim.x) {
      best_triplets[j] = -1;
    }
    ...
    for (uint8_t relative_first_layer = 0; relative_first_layer < 4; ++relative_first_layer) {
      ...
      lf_triplet_seeding_impl(
        ...
        best_chi2 + relative_first_layer * LookingForward::maximum_number_of_candidates,
        best_h0_h2 + relative_first_layer * 2 * LookingForward::maximum_number_of_candidates,
        scifi_lf_candidates,
        z0,
        z1,
        z2,
    ...
    for (uint16_t j = threadIdx.x; j < LookingForward::maximum_number_of_candidates_per_ut_track; j += blockDim.x) {
      ...
      if (k != -1) {
        ...
        const auto h0_offset = (relative_middle_layer - 1) * 2 * LookingForward::maximum_number_of_candidates;
        const auto h2_offset = h0_offset + LookingForward::maximum_number_of_candidates;
        ...
        const uint16_t h0 = (uint16_t) scifi_lf_candidates[(relative_middle_layer - 1) * LookingForward::maximum_number_of_candidates + best_h0_h2[h0_offset + element]];
