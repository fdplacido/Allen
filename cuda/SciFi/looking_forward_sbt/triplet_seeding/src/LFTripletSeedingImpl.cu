...
// Tensor core specialization
  ...
  const half big_max_chi2 = 10000.f * LookingForward::chi2_max_triplet_single;
  ...
  for (int k = threadIdx.x; k < LookingForward::tile_size * LookingForward::tile_size; k += blockDim.x) {
    const int8_t h0_rel = k % LookingForward::tile_size;
    const int8_t h2_rel = k / LookingForward::tile_size;
  ...
  for (int h1_all_threads = threadIdx.x; h1_all_threads < LookingForward::maximum_number_of_triplets_per_h1 *
    ...
    if (h1_rel < h1_candidate_size) {
      ...
      if (best_k != -1) {
        best_combined[h1_rel + side * LookingForward::maximum_number_of_candidates] = SciFi::CombinedValue {
          best_chi2, (int16_t)(best_k % LookingForward::tile_size), (int16_t)(best_k / LookingForward::tile_size)};
      }
    }
  }
#endif
