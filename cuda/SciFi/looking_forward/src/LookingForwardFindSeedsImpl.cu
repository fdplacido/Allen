  ...
  SciFi::TrackCandidate* scifi_track_candidates)
{
  ...
  if (
    ...
    for (auto hit_layer_0_it = std::get<0>(layer0_candidates); !track_limit_surpassed && hit_layer_0_it != std::get<1>(layer0_candidates); hit_layer_0_it++) {
      const auto projected_slope = (x_mag - hits.x0[hit_layer_0_it]) / (z_mag - proj_states[0].z);
      proj_states[3].x = linear_propagation(hits.x0[hit_layer_0_it], projected_slope, proj_states[3].z - proj_states[0].z);
      ...
      for (auto hit_layer_3_it = std::get<0>(layer3_candidates); !track_limit_surpassed && hit_layer_3_it != std::get<1>(layer3_candidates); hit_layer_3_it++) {
        const auto slope_layer_3_layer_0 =
          (hits.x0[hit_layer_0_it] - hits.x0[hit_layer_3_it]) / (LookingForward::dz_x_layers);
        ...
        for (auto hit_layer_1_it = std::get<0>(layer1_candidates); !track_limit_surpassed && hit_layer_1_it != std::get<1>(layer1_candidates); hit_layer_1_it++) {
          ...
          for (auto hit_layer_2_it = std::get<0>(layer2_candidates); !track_limit_surpassed && hit_layer_2_it != std::get<1>(layer2_candidates); hit_layer_2_it++) {
