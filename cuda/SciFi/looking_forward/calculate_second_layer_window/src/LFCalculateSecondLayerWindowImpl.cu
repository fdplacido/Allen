  ...
  const uint total_number_of_candidates)
{
  ...
  for (int i = threadIdx.y; i < size_first_candidate; i += blockDim.y) {
    ...
    layer_3_projected_state.x =
      linear_propagation(hit_layer_0_x, projected_slope, layer_3_projected_state.z - looking_forward_constants[0]);

    const auto layer3_offset_nhits =
      get_offset_and_n_hits_for_layer(2 * (first_layer + 3), hit_count, layer_3_projected_state.y);
    ...
    second_candidate_p[3 * total_number_of_candidates + i] =
  ...
  for (int i = threadIdx.y; i < size_first_candidate; i += blockDim.y) {
    ...
    const auto slope_layer_3_layer_0_minimum =
      (hits.x0[second_candidate_p[2 * total_number_of_candidates + i] + hit_count.event_offset()] - hit_layer_0_x) /
      (LookingForward::dz_x_layers);
    const auto slope_layer_3_layer_0_maximum =
      (hits.x0
         [second_candidate_p[2 * total_number_of_candidates + i] +
          second_candidate_p[3 * total_number_of_candidates + i] - 1 + hit_count.event_offset()] -
       hit_layer_0_x) /
      (LookingForward::dz_x_layers);
    ...
    const auto layer1_offset_nhits =
      get_offset_and_n_hits_for_layer(2 * (first_layer + 1), hit_count, layer_1_projected_state_y);
    ...
    second_candidate_p[5 * total_number_of_candidates + i] =
    ...
    const auto layer2_offset_nhits =
      get_offset_and_n_hits_for_layer(2 * (first_layer + 2), hit_count, layer_2_projected_state_y);
    ...
    second_candidate_p[7 * total_number_of_candidates + i] =
