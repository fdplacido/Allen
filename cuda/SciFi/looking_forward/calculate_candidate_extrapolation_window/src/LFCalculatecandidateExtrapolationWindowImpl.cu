...
{
  // do the propagation
  const auto x_at_layer_8 = hits.x0[track_candidate.hits[0]];
  const auto reco_slope = (hits.x0[track_candidate.hits[1]] - x_at_layer_8) * LookingForward::inverse_dz_x_layers;
