...
__global__ void muon_decoding(const uint* event_list, const char* events, const unsigned int* offsets,
                              MuonRawToHits* muon_raw_to_hits, HitsSoA* muon_hits, const uint number_of_events) {
  
  for (int event_number = 0; event_number < number_of_events; ++event_number) {
    __shared__ uint currentHitIndex;
    const size_t eventId = event_list[event_number];
    const size_t output_event = event_number;
    ...
    //       const auto stationRegionQuarter = MuonTileID::stationRegionQuarter(tileId);
    //       atomicAdd(storageStationRegionQuarterOccurrencesOffset + stationRegionQuarter + 1, 1);
    //     }
    //   }
    // }
    ...
    if (threadIdx.x == 0) {
      ...
      for (int i=0; i < Muon::Constants::n_stations * Muon::Constants::n_regions * Muon::Constants::n_quarters; ++i) {
