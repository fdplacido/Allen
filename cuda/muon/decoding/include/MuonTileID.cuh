...
namespace Muon {
  ...
  public:
    __device__ MuonTileID(unsigned int muonid) {
      m_muonid = muonid;
    }

    __device__ MuonTileID() {
      m_muonid = 0;
    }
    ...
    __device__ unsigned int station() const {
    ...
    __device__ void setX(const unsigned int x) {
      set(x, MuonBase::ShiftX, MuonBase::MaskX);
    }

    __device__ void setY(const unsigned int y) {
    ...
    __device__ unsigned int id() const {
      return m_muonid;
    }
  };
}
