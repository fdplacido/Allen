  ...
  const uint number_of_events)
{
  ...
  for (int event_number = 0; event_number < number_of_events; ++event_number) {
    ...
    for (int i = threadIdx.x; i < Muon::Constants::n_stations * Muon::Constants::n_regions * Muon::Constants::n_quarters;
      ...
      const auto offset = event_number * Muon::Constants::n_stations * Muon::Constants::n_regions * Muon::Constants::n_quarters;
      const auto start_index = dev_storage_station_region_quarter_offsets[offset + i];
      const auto end_index = dev_storage_station_region_quarter_offsets[offset + i + 1];

      // TODO: We are fetching the first tile ID
      //       We should verify this logic holds (it does not atm)
      const auto tile = Muon::MuonTileID(dev_storage_tile_id[start_index]);
      ...
      printf("Start and end index: %i, %i\n", start_index, end_index);
      for (int j = start_index; j < end_index; ++j) {
        float x = 0.f, dx = 0.f, y = 0.f, dy = 0.f, z = 0.f, dz = 0.f;
        const auto tile = Muon::MuonTileID(dev_storage_tile_id[j]);
        const auto region = tile.region();
        ...
        // else {
        //   if (Muon::MuonTileID::layout(dev_storage_tile_id[j]) == layout_one) {
        ...
        const auto localCurrentHitIndex = atomicAdd(dev_atomics_muon + number_of_events + event_number, 1);
