  ...
  const uint* dev_unique_x_sector_layer_offsets)
{
  ...
  for (uint i = threadIdx.x; i < number_of_tracks_event; i += blockDim.x) {
    ...
    dev_ut_x[event_tracks_offset + i]  = event_veloUT_tracks[i].x;
    dev_ut_z[event_tracks_offset + i]  = event_veloUT_tracks[i].z;
    dev_ut_tx[event_tracks_offset + i] = event_veloUT_tracks[i].tx;
