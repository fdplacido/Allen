  ...
  const bool* dev_is_muon)
{
  ...
  const uint event_tracks_offset = scifi_tracks.tracks_offset(event_number);
  const uint event_velo_tracks_offset = velo_tracks.tracks_offset(event_number);
  const uint n_scifi_tracks = scifi_tracks.number_of_tracks(event_number);
  
  const Associate::Consolidated::Table velo_pv_ip {dev_velo_pv_ip, velo_tracks.total_number_of_tracks};
  const auto pv_table = velo_pv_ip.event_table(scifi_tracks, event_number);
  ...
  for (int i_track = threadIdx.x; i_track < n_scifi_tracks; i_track += blockDim.x) {
    ...
    for (int j_track = threadIdx.y + i_track + 1; j_track < n_scifi_tracks; j_track += blockDim.y) {
      ...
      // Preselection on second track.
      const int j_ut_track = scifi_tracks.ut_track[j_track];
      const int j_velo_track = ut_tracks.velo_track[j_ut_track];
      const ParKalmanFilter::FittedTrack trackB(
        kalmanvelo_states.get(event_velo_tracks_offset + j_velo_track),
        scifi_tracks.qop[j_track],
        dev_is_muon[event_tracks_offset + j_track]);
      if (trackB.pt() < VertexFit::trackMinPt || (trackB.ipChi2 < VertexFit::trackMinIPChi2 && !trackB.is_muon)) {
        continue;
      }
      ...
      // Only combine tracks from the same PV.
      if (
        pv_table.pv[i_velo_track] != pv_table.pv[j_velo_track] &&
        pv_table.value[i_velo_track] > VertexFit::maxAssocIPChi2 &&
        pv_table.value[j_velo_track] > VertexFit::maxAssocIPChi2) {
        continue;
      }
      ...
      const int vertex_idx = (int) n_scifi_tracks * ((int) n_scifi_tracks - 3) / 2 -
        ((int) n_scifi_tracks - 1 - i_track) * ((int) n_scifi_tracks - 2 - i_track) / 2 + j_track;
      if (n_pvs_event > 0) {
        int ipv = pv_table.value[i_velo_track] < pv_table.value[j_velo_track] ?
          pv_table.pv[i_velo_track] : pv_table.pv[j_velo_track];
        auto pv = vertices[ipv];
        fill_extra_pv_info(event_secondary_vertices[vertex_idx], pv, trackA, trackB);
      }
      else {
        // Set the minimum IP chi2 to 0 by default.
        event_secondary_vertices[vertex_idx].minipchi2 = 0;
      }
    }
  }
