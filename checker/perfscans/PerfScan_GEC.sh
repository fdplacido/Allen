...
do
  ...
  cd build
  make -j 8 >& /tmp/WTF
  ./Allen -f /data/gligorov/minbias -n 1000 -c 1 -m 3000 >& $outloc\/GEC-$val_low-$val_hi-scanmb.stdout
...
do
  ...
  cd build
  make -j 8 >& /tmp/WTF
  ./Allen -f /data/gligorov/signals/Bs2PhiPhi/mag_down -n 1000 -c 1 -m 3000 >& $outloc\/GEC-$val_low-$val_hi-scansig.stdout
  cd ..
  cp bkpfile.bkp cuda/global_event_cut/include/GlobalEventCutConfiguration.cuh
done
