...
outputfile = ROOT.TFile("../../../plotsfornote_root/muon_id_efficiency_plots.root", "recreate")
...
for category in muonCategories:
    ...
    for histo in efficiencyHistos:
        ...
        ROOT.gPad.SetTicks()
        numeratorName = "Forward/" + muonCatDict[category]["numerator"] + efficiencyHistoDict[histo]["variable"] + "_reconstructed"
        print("Opening " + numeratorName)
        numerator = f.Get(numeratorName)
        denominatorName = "Forward/" + muonCatDict[category]["denominator"] + efficiencyHistoDict[histo]["variable"] + "_reconstructible"
