...
reshist = {}
for coord in ["x","y","z"] :
  reshist[coord] = ROOT.TH2F("reshist"+coord,"reshist"+coord,15,0,150,20,-1.*ranges[coord],ranges[coord])

for entry in range(t.GetEntries()) :
  t.GetEntry(entry)
  for coord in ["x","y","z"] :
    reshist[coord].Fill(t.ntrinmcpv,1000.*t.__getattr__("diff_"+coord))

arr = ROOT.TObjArray()
for coord in ["x","y","z"] :
  pvcanv[coord] = ROOT.TCanvas("pvcanv"+coord,"pvcanv"+coord,900,800)
  pvcanv[coord].cd(1)
  reshist[coord].FitSlicesY(ROOT.TF1("gausx","gaus(0)",-1.*ranges[coord],ranges[coord]),0,-1,0,"QNR",arr)
