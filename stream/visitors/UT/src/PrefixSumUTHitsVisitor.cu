...
void SequenceVisitor::set_arguments_size<prefix_sum_ut_hits_t>(
  const prefix_sum_ut_hits_t& state,
  prefix_sum_ut_hits_t::arguments_t arguments,
  const RuntimeOptions& runtime_options,
  const Constants& constants,
  const HostBuffers& host_buffers)
{
  arguments.set_size<dev_prefix_sum_auxiliary_array_3>(prefix_sum_ut_hits_t::aux_array_size(
  ...
  cudaEvent_t& cuda_generic_event)
{
  ...
  // // Check that
  // cudaCheck(cudaMemcpyAsync(host_ut_hit_count.data(), arguments.offset<dev_ut_hit_offsets>(),
  // arguments.size<dev_ut_hit_offsets>(), cudaMemcpyDeviceToHost, stream)); cudaEventRecord(cuda_generic_event,
  // stream); cudaEventSynchronize(cuda_generic_event); for (int e=0; e<number_of_events; ++e) {
