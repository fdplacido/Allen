...
{
  arguments.set_size<dev_prefix_sum_auxiliary_array_9>(
    muon_station_ocurrence_prefix_sum_t::aux_array_size(arguments.size<dev_station_ocurrences_offset>() /
        sizeof(dev_station_ocurrences_offset::type)));
}
  ...
  cudaEvent_t& cuda_generic_event)
{
  if (runtime_options.cpu_offload) {
    ...
    state.set_arguments(
      arguments.offset<dev_station_ocurrences_offset>(),
      arguments.offset<dev_prefix_sum_auxiliary_array_9>());
    ...
    cudaCheck(cudaMemcpyAsync(
      host_buffers.host_muon_total_number_of_hits,
      arguments.offset<dev_station_ocurrences_offset>()
      + (arguments.size<dev_station_ocurrences_offset>() >> 2) - 1,
      sizeof(int),
