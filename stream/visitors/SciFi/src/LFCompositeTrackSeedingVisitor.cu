  ...
  const HostBuffers& host_buffers)
{
  ...
  arguments.set_size<dev_scifi_lf_triplet_best_h0h2>(
    host_buffers.host_number_of_reconstructed_ut_tracks[0] * 4 * 2 * LookingForward::maximum_number_of_candidates);
  ...
  cudaEvent_t& cuda_generic_event)
{
  ...
  state.handler_lf_triplet_seeding.set_opts(
    dim3(host_buffers.host_number_of_selected_events[0]), dim3(32), cuda_stream); // to do: check gridDim.y = 32
