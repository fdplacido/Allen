...
void SequenceVisitor::set_arguments_size<scifi_pre_decode_v5_t>(
  scifi_pre_decode_v5_t::arguments_t arguments,
...
void SequenceVisitor::visit<scifi_pre_decode_v5_t>(
  ...
  const scifi_pre_decode_v5_t::arguments_t& arguments,
  const RuntimeOptions& runtime_options,
  const Constants& constants,
...
{
  state.set_opts(
    dim3(host_buffers.host_number_of_selected_events[0]), dim3(1), cuda_stream); //SciFi::SciFiRawBankParams::NbBanks statt 1
  state.set_arguments(
  ...
  std::ofstream outfile("dump_predecode.txt");
