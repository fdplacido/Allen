...
void HostBuffers::reserve(const uint max_number_of_events) {
  ...
  cudaCheck(cudaMallocHost((void**)&host_ut_tracks, max_number_of_events * UT::Constants::max_num_tracks * sizeof(UT::TrackHits)));
  ...
  cudaCheck(cudaMallocHost((void**)&host_scifi_tracks, max_number_of_events * SciFi::Constants::max_tracks * sizeof(SciFi::TrackHits)));
  ...
  cudaCheck(cudaMallocHost((void**)&host_scifi_track_hit_number, max_number_of_events * SciFi::Constants::max_tracks * sizeof(uint)));
  cudaCheck(cudaMallocHost((void**)&host_scifi_track_hits, max_number_of_events * SciFi::Constants::max_tracks * SciFi::Constants::max_track_size * sizeof(SciFi::Hit) ));
  cudaCheck(cudaMallocHost((void**)&host_scifi_qop, max_number_of_events * SciFi::Constants::max_tracks * sizeof(float))); 
...
size_t HostBuffers::ut_track_hit_number_size() const {
  return host_number_of_reconstructed_ut_tracks[0] + 1;
}

size_t HostBuffers::scifi_track_hit_number_size() const {
  return host_number_of_reconstructed_scifi_tracks[0] + 1;
}
