...
bool select_hits(const MiniState &velo_UT_state, float UT_qop, unsigned int UT_track_index,const SciFi::Hits &hits,
                 const SciFi::HitCount & hit_count, const int station,
                 std::vector<SciFi::TrackHits> &track_candidate, const SciFiWindowsParams &window_params){
    bool ret_val = false;
    ...
    if ((proj_state[0].x > SciFi::LookingForward::xMin && proj_state[0].x < SciFi::LookingForward::xMax) &&
        ...
        find_x_in_window(hits, n_hits[0], zone_offset[0], proj_state[0].x-dx_plane_0, proj_state[0].x+dx_plane_0, min_idx[0], max_idx[0]);
        ...
        for (auto hit_layer_0_it = min_idx[0] ; hit_layer_0_it != max_idx[0] ; hit_layer_0_it++){
            ...
            proj_state[3].x = linear_propagation(hits.x0[hit_layer_0_it], projected_slope,
                                           proj_state[3].z - proj_state[0].z);
            ...
            find_x_in_window(hits, n_hits[3], zone_offset[3],
                    proj_state[3].x - window_params.max_window_layer3,
                    proj_state[3].x + window_params.max_window_layer3,
                    min_idx[3], max_idx[3]);
            // track_candidate.window_stats[11].emplace_back(Window_stat(max_it[3] - min_it[3], x_proj[3], window_params.max_window_layer3));
            for (auto hit_layer_3_it = min_idx[3] ; hit_layer_3_it != max_idx[3] ; hit_layer_3_it++){
                const float slope_layer_3_layer_0 = (hits.x0[hit_layer_0_it] - hits.x0[hit_layer_3_it])/(SciFi::LookingForward::dz_x_layers);
                // TODO add layer[2]
                float hit_on_layer_1;
                float hit_on_layer_2;
                proj_state[1].x = linear_propagation(hits.x0[hit_layer_0_it], slope_layer_3_layer_0,
                ...
                find_x_in_window(hits, n_hits[1], zone_offset[1],
                        proj_state[1].x - window_params.max_window_layer1,
                        proj_state[1].x + window_params.max_window_layer1,
                        min_idx[1], max_idx[1]);

                // track_candidate.window_stats[9].emplace_back(Window_stat(max_it[1] - min_it[1], x_proj[1], window_params.max_window_layer3));

                proj_state[2].x = linear_propagation(hits.x0[hit_layer_0_it], slope_layer_3_layer_0,
                ...
                find_x_in_window(hits, n_hits[2], zone_offset[2],
                        proj_state[2].x - window_params.max_window_layer2,
                        proj_state[2].x + window_params.max_window_layer2,
                        min_idx[2], max_idx[2]);

                // track_candidate.window_stats[10].emplace_back(Window_stat(max_it[2] - min_it[2], x_proj[2], window_params.max_window_layer2));
                ...
                for (auto hit_layer_1_it = min_idx[1] ; hit_layer_1_it != max_idx[1] ; hit_layer_1_it++){
                    ...
                    y_layer_1 = (hits.x0[hit_layer_0_it] + slope_layer_3_layer_0*(SciFi::LookingForward::dz_x_u_layers) -
                                 hits.x0[hit_layer_1_it]) / SciFi::LookingForward::Zone_dxdy[1];
                    ...
                    for (auto hit_layer_2_it = min_idx[2] ; hit_layer_2_it != max_idx[2] ; hit_layer_2_it++){
                        ...
                        y_layer_2 = (hits.x0[hit_layer_0_it] + slope_layer_3_layer_0*(SciFi::LookingForward::dz_x_v_layers) -
                                     hits.x0[hit_layer_2_it]) / SciFi::LookingForward::Zone_dxdy[2];
...
void find_x_in_window(const SciFi::Hits &hits, const int num_hits, const int zone_offset,
                      const float x_min, const float x_max, int &offset_begin, int &offset_end){
    // TODO implement something smarter like a binary search
    for (offset_begin = zone_offset ; hits.x0[offset_begin] < x_min && offset_begin < (zone_offset+num_hits); offset_begin++);
