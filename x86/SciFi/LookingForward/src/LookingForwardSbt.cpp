  ...
  const float chi2_extrap_stddev)
{
  ...
  // do the propagation
  const auto h0 = track.hits[0];
  const auto h1 = track.hits[1];
  const auto layer0 = scifi_hits.planeCode(h0) / 2;
  const auto layer1 = scifi_hits.planeCode(h1) / 2;
  const auto x_at_layer_0 = hits.x0[h0];
  const auto x_at_layer_1 = hits.x0[h1];
  ...
  const auto reco_slope = (x_at_layer1 - x_at_layer_0) / (z_at_layer_1 - z_at_layer_0);
  const auto projection_x = scifi_propagation(
                              x_at_layer_0,
                              reco_slope,
                              track.qop,
  ...
  if (best_idx != -1) {
    track.add_hit_with_quality(best_idx, layer);
  }
}
