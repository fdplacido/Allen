    ...
    for(int i_veloUT_track = 0; i_veloUT_track < n_veloUT_tracks_event; ++i_veloUT_track ) {
      ...
      for ( int i_hit = 0; i_hit < n_hits_x; ++i_hit ) { 
        ...
        int ret_qop = update_qop_estimate(
          UT_state, qop,
          x_1, scifi_params_T1, 
          xf_t1, yf_t1, txf_t1, tyf_t1, der_xf_qop_t1, qop_update);
        ...
        for ( int i_hit_other = 0; i_hit_other < n_hits_x_other; ++i_hit_other ) { 
          ...
          if ( x_1-x_3 > SciFi::MomentumForward::x_diff_layer_qop_offset - slope1 * qop_update 
               || x_1-x_3 < -1.f * SciFi::MomentumForward::x_diff_layer_qop_offset - slope2 * qop_update) continue;
