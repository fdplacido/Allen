    ...
    for (int i_veloUT_track = 0; i_veloUT_track < n_veloUT_tracks_event; ++i_veloUT_track) {
      ...
      if (ret) {
        ...
        if (true_scifi_ids.size() > 0) {
          ...
          if (match_t1) {
            ...
            if (i_event < 100) {
              output_pierre << qop << "\t" << UT_x << "\t" << UT_tx << "\t" << UT_y << "\t" << UT_ty << "\t" << UT_z
                            << "\t" << 1. / p_true << "\t" << true_x_t1 << std::endl;
            }

            int ret_qop = update_qop_estimate(
              UT_state, qop, true_x_t1, scifi_params_T1, xf_t1, yf_t1, txf_t1, tyf_t1, der_xf_qop_t1, qop_update_t1);
            ...
            for (const auto true_id : true_scifi_ids) {
              for (int i_hit = 0; i_hit < n_hits_other; ++i_hit) {
                ...
                if (true_id == lhcbid) {
                  ...
                  match_t1_other = true;
                  tx_x_hits_t1 = (true_x_t1_other - true_x_t1) /
                                 SciFi::MomentumForward::dz_x_layers; // dz of x-layers within one station = 210 mm
                  break;
                }
              }
        ...
        for (int i_hit = 0; i_hit < n_hits; ++i_hit) {
          ...
          if (
            fabsf(x - xf_t1) < SciFi::MomentumForward::dx_extrap_qop_offset_T1 +
                                 SciFi::MomentumForward::dx_extrap_qop_slope_T1 * fabsf(qop)) {
            ...
            for (int i_hit_other = 0; i_hit_other < n_hits_other; ++i_hit_other) {
              ...
              if (
                fabsf(x - x_other) < SciFi::MomentumForward::x_diff_layer_qop_offset - slope1 * qop &&
                ...
                if (std::abs(tx_hits - txf_t1) < SciFi::MomentumForward::max_tx_diff) n_hits_in_window_other_t1_tx++;
              }
            }
      ...
      if (ret) {
        ...
        if (true_scifi_ids.size() > 0) {
          ...
          if (match_t3) {
            int ret_qop = update_qop_estimate(
              UT_state, qop, true_x_t3, scifi_params_T3, xf_t3, yf_t3, txf_t3, tyf_t3, der_xf_qop_t3, qop_update_t3);
            ...
            for (const auto true_id : true_scifi_ids) {
              for (int i_hit = 0; i_hit < n_hits_other; ++i_hit) {
                ...
                if (true_id == lhcbid) {
                  ...
                  tx_x_hits_t3 = (true_x_t3_other - true_x_t3) /
                                 SciFi::MomentumForward::dz_x_layers; // dz of x-layers within one station = 210 mm
                  match_t3_other = true;
                  break;
                }
        ...
        for (int i_hit = 0; i_hit < n_hits; ++i_hit) {
          ...
          const float x = scifi_hits.x0[hit_index];
          if (
            fabsf(x - xf_t3) < SciFi::MomentumForward::dx_extrap_qop_offset_T3 +
                                 SciFi::MomentumForward::dx_extrap_qop_slope_T3 * fabsf(qop)) {
