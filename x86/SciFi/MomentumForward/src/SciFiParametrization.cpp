...
int Coef::Read(FILE* inp, int degx1, int degx2, int degy1, int degy2)
{
  ...
  for (int i = 0; i < degx1; i++) {
    fscanf(inp, "%lf", &a);
    tx10.push_back(1e-3 * a);
  }
  for (int i = 0; i < degx1; i++) {
    fscanf(inp, "%lf", &a);
    tx01.push_back(1e-3 * a);
  }
  ...
  for (int i = 0; i < degy1; i++) {
    fscanf(inp, "%lf", &a);
    ty10.push_back(1e-3 * a);
  }
  for (int i = 0; i < degy1; i++) {
    fscanf(inp, "%lf", &a);
    ty01.push_back(1e-3 * a);
  }
...
SciFi::Parameters::Parameters(const char* name)
{
  ...
  ;
  debug_cout << "BENDX " << BENDX << " BENDX_X2 " << BENDX_X2 << " BENDX_Y2 " << BENDX_Y2 << " BENDY_XY " << BENDY_XY
             << std::endl;
  ...
  for (int ix = 0; ix < Nbinx; ix++)
    for (int iy = 0; iy < Nbiny; iy++)
      C[ix][iy].Read(coef, DEGX1, DEGX2, DEGY1, DEGY2);
...
// little bit more computations) XGridO[tion and YGridOption describe the choice of xy grid. By default, it is 1
// (equally spaced values)
{
  ...
  ux = (txi - xi / zi - bendx * qop) / params.Dtxy;
  uy = (tyi - yi / zi - bendy * qop) / params.Dtxy;
  ...
  if (params.QuadraticInterpolation) {
    ...
    c00 = params.C[ix][iy];
    cp0 = params.C[ix + 1][iy];
    c0p = params.C[ix][iy + 1];
    c0n = params.C[ix][iy - 1];
    cn0 = params.C[ix - 1][iy];
  ...
  for (int deg = 0; deg < c.Degx2; deg++) {
    double coef = c.x00[deg];
    if (deg < c.Degx1) coef += c.x10[deg] * ux + c.x01[deg] * uy;
  ...
  float& qop_update)
{
  ...
  for (int i = 0; i < MAXITER; ++i) {
    if (i > 0) {
      int ret = extrap(UT_state.x, UT_state.y, UT_state.tx, UT_state.ty, r_prev, params, xf, yf, txf, tyf, der_xf_qop);
      if (!ret) return 0;
    }
    qop_update = r_prev + (xhit - xf) / der_xf_qop;
    // debug_cout << "at iteration " << i << ": r_prev = " << std::setprecision(6) << r_prev << ", current r = " <<
    // qop_update << ", r difference = " << qop_update-r_prev << ", xf = " << xf << ", xhit = " << xhit << std::endl;
    // debug_cout << "der_xf_qop = " << der_xf_qop << std::endl;
    if (std::abs(xf - xhit) < XCONVERGENCE) return 1;
    r_prev = qop_update;
  }
  return 0;
}
